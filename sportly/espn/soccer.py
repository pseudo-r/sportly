"""sportly.espn.soccer — 260+ leagues worldwide.

Usage::

    from sportly.espn import soccer
    table   = soccer.standings("eng.1")   # EPL table
    scorers = soccer.top_scorers("eng.1")
...
def standings(league: str = DEFAULT_LEAGUE, *, season: int | None = None, client: SportlyClient | None = None) -> Standings:
    """Full table — uses /apis/v2/ (site/v2 returns empty {} for soccer)."""
...
def play_by_play(event_id: str, league: str = DEFAULT_LEAGUE, *, limit: int = 300, client: SportlyClient | None = None) -> dict[str, Any]:
    """Match play-by-play (goals, cards, substitutions)."""
...
def top_scorers(league: str = DEFAULT_LEAGUE, *, season: int | None = None, client: SportlyClient | None = None) -> dict[str, Any]:
    """Top scorers / statistical leaders."""
